...
<script>
  ...
  export default {
      ...
      'Config.datasource': {
        handler(curVal, oldVal){
          ...
          this.uCanvas.on('click', function (param) {
            that.setMultiLineColor(param.seriesIndex)
          })
        },
        deep: true
      },
      'Config.isSelected': {
        handler(curVal, oldVal){
          ...
          if (curVal) {
            ...
            propertyObject.type = 'uLineMulti-linestyle'
    ...
    methods: {
       ...
       * 设置多色柱的颜色
       */
      setMultiLineColor(dindex) {
        setTimeout(() => {
          let propertyArr = []
          let propertyObject = $.extend(true, {}, PropConfig.props)
          propertyObject.type = 'uLineMulti-linecolor'
